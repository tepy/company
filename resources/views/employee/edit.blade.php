                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('employee.update', ['employee' => $employee->id]) }}" >
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <input id="email" type="email" class="form-control @error('email') is-invalid @enderror" name="email" required value="{{ $employee->email }}">
                            </div>
                        </div>

                        <div class="form-group row">
                            <label for="password" class="col-md-2 col-form-label text-md-right">Perusahaan</label>

                            <div class="col-md-10">
                                <select name="company_id" class="form-control" required="">
                                    <option value="">--Silahkan pilih--</option>
                                    @foreach($company as $val)
                                        <option @if($val->id == $employee->company_id) selected="" @endif value="{{ $val->id }}">{{ $val->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
